...
## 8. 改了桥接源码，**工具还是报旧错误** `[通病]`

**症状**：你在磁盘上明明已经把 bug 修好了（甚至三份副本都对比过，内容一致），
但**通过宿主调用那个工具，报的还是修复前的错**。改成正确写法后错误依旧，
于是开始怀疑自己的修复方式不对 —— 实际上是**进程没换**。

**根因**：宿主启动时就 spawn 了 MCP 服务进程，**它加载的是那一刻的模块**。
之后改文件不会影响已在运行的进程（Node 不会热重载 `import` 过的模块）。
宿主对 MCP 服务的注册表与进程**只在启动时组合一次**（这也是第 6 节的规律）。

**判据（30 秒区分"代码没改对"还是"进程是旧的"）**：

```
1. 对比文件 mtime 与宿主进程的启动时间
   Get-Item <bridge.mjs> | Select LastWriteTime
   Get-Process | Where-Object { $_.ProcessName -like "*node*" } | Select Id,StartTime
   → 文件比进程新 ⇒ 进程跑的是旧代码，别再看代码了

2. 全盘找所有副本，确认你改的是宿主真正加载的那份
   宿主配置里通常写的是绝对路径（如 <DSH_HOME>\tools\<name>.mjs），
   而"源码目录里的那份"可能只是个待部署的模板 —— 两份不同步是常态。
   Get-ChildItem <多个根> -Recurse -Filter <name>.mjs | Select FullName,Length,LastWriteTime

3. 在源码里加一行临时 stderr 打印，重启宿主后再看有没有出现
   没出现 ⇒ 100% 是没加载新代码
```

**解法**：重启宿主（或宿主提供的 MCP 重连/重载入口）。
派生的教训：**修完桥接器要立刻验证一次"错误变了没有"**，
如果错误文案一模一样，先按上面的判据查进程，再回头怀疑修复逻辑。

**顺带的工程规范**：宿主配置指向的**部署副本**与**源码目录**必须有一套同步机制
（本项目用 `publish.ps1` 显式拷贝），否则"改了没生效"会反复发生，
而每次都以为是代码问题，白白浪费一轮排查。

**验证**：重启后重放**同一个**失败调用，确认错误信息发生变化（或直接成功）。
如果完全没变，说明你重启的不是承载该 MCP 的那个宿主进程。
