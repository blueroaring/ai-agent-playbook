...
### 18.1 目标**自己有碰撞**时，交互区必须往外扩

玩家走不进带碰撞的家具/墙体里，所以"交互区 = 物件占地"等于**按不动**。
正确做法是**外扩一圈**（本例 24px），让站在旁边/站在房内第一排的玩家够得到；
挂在**墙上**的东西（窗户、门）尤其要往房间内侧延伸。
判据：`E` 按了没反应，但提示框显示"[E] 查看 XX" —— 那就是探到了、够不到，十有八九是没外扩。

```gdscript
rs.size = rect.size + Vector2(MARGIN, MARGIN) * 2.0   # 不是 rect.size
```
